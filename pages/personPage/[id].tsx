...
const person = ({initialMobxState}:any) => {
    ...
    return(
        <Box sx={{mb:'4rem', '@media (max-width: 1200px)': {}}}>
              ...
              <BackButton/>
                    <Container maxWidth={false} sx={{ width: '37.25rem', '@media (max-width: 1200px)': {width:'58rem'}, '@media (max-width: 876px)': {width:'100%', mt:'2rem'}}}>
                        {store.person &&
                        <>
                    <Box sx={{borderRadius:'20px', overflow:'hidden',width:'7.5rem', height:'7.5rem', position:'relative', '@media (max-width: 876px)': {width:'12rem', height:'12rem'}}}>
                        <Image fill style={{ objectFit: 'cover'}} src={store.person.person.posterUrl} alt={store.person.person.nameRu} />
                    </Box>
                    <Box sx={{mt:'1rem', '@media (max-width: 876px)': {mt:'2rem'}}}>
                    ...
                    <Stack direction='row' alignItems='center' gap={1} sx={{mt:'3rem'}}>
                        ...
                        <MyText text={`${store.person.movies.length} ${getNoun(store.person.movies.length, 'фильм', 'фильма', 'фильмов')}`} size="1.25rem"/>
                    </Stack>
                    ...
                    {store.person.movies.map(movie=>{
                        return(
                        <Box key={movie.id} sx={{ '@media (max-width: 876px)': {
                            mt:'2rem'
                        }}}>
                            <Link
                                href={{
                                ...
                                <Box
                                    sx={{display:'flex', marginTop:'1rem', marginBottom:'1rem', alignItems:'center', '@media (max-width: 576px)': {
                                        alignItems:'stretch'
                                     }}}
                                >
                                <Image
                                    ...
                                    alt={movie.nameRu}
                                ></Image>
                                <Box sx={{display:'flex', width:'100%', flexDirection:'row', '@media (max-width: 576px)': {
                                               flexDirection:'column',
                                               justifyContent:'space-between',
                                               ml:'1rem'
                                            }}}>
                                        <Box flexGrow={1} sx={{pl:'1rem', '@media (max-width: 576px)': {
                                                pl:'unset'
                                            }}}>
                                        <Box> 
                                    ...
                                    <MyButton text="Подробнее" hoverColor='#3e3659' width="8.75rem" mobileWidth="100%"/>
                                </Box>
                                </Box>
...
export async function getServerSideProps({query, res, locale}) {
    ...
    const personUrl = `http://192.168.0.103:3003/info/person/${id}`;
