 ...
 */
  ...
  const sortTypes = [{nameRu:'По количеству оценок', name:null, id:'ratingKinopoiskVoteCount-DESC'},
                    {nameRu:'По рейтингу', name:null, id:'ratingKinopoisk-DESC'},
                    {nameRu:'По дате выхода', name:null ,id:'year-DESC'},
                    {nameRu:'По алфавиту', name:null ,id:translationStore?.currentLanguage==='ru' ? 'nameRu-ASC' : 'nameEn-ASC'}];
...
<>
            ...
            <Box sx={{width:'70%', mt:'2rem'}}>
              ...
              <DropDownItem 
                  button
                  text={rootStore.moviesStore.selectedFilters.order[0]?.name ||  'Сортировка'} 
                  name="order"
                  content={sortTypes}
                  isTransparent={true}
                ...
                {urlQueries.slug ? (
                  <>
                    <MoviesList url={urlQueries.slug} movies={initialMobxState.movies.rows}/>
...
export async function getServerSideProps({req, res, locale}) {
  ...
  const moviesUrl = `http://192.168.0.103:3003/info?limit=${rootStore.moviesStore.limit}&page=${rootStore.moviesStore.page}`
